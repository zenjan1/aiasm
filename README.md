...
专为 AI 设计的最小汇编语言工具链，包含一个从零编写的 32 位 x86 交互操作系统内核，内置 WASM 字节码运行时引擎。基于 GNU AS 汇编语法（Intel 模式），零依赖，纯汇编实现。
...
- [内核架构](#内核架构)
- [v0.7 新特性](#v07-新特性)
  - [WASM 字节码解析器](#wasm-字节码解析器)
  - [WASM 虚拟机](#wasm-虚拟机)
  - [WASM 系统调用桥接](#wasm-系统调用桥接)
  - [WASM 应用示例](#wasm-应用示例)
  - [物理内存管理](#物理内存管理)
  - [虚拟内存与分页](#虚拟内存与分页)
  - [进程管理与调度](#进程管理与调度)
...
- **WASM 运行时**：内置 WASM 字节码解析器 + 虚拟机，支持栈式执行、控制流（block/loop/if/br/br_if/br_table）、局部变量、线性内存、宿主函数调用
...
```asm
mov     esi, offset wasm_module    # esi = WASM 模块指针
mov     ecx, wasm_module_size      # ecx = 模块大小
call    wasm_parse_module          # 解析模块
call    wasm_load_data             # 加载数据段到线性内存
xor     eax, eax
call    wasm_exec_func             # 执行函数 0 → eax = 返回值
```

- 解析 WASM 二进制格式：魔数、版本、类型段、函数段、代码段、数据段
- 代码体指针表，支持多函数索引调用
- LEB128 编码整数（无符号和有符号）

### WASM 虚拟机

- **栈式执行模型**：256 元素操作数栈，所有指令通过栈传递数据
- **控制流**：block、loop、if/else、end、br、br_if、br_table
- **变量操作**：local.get、local.set、local.tee、global.get、global.set
- **内存操作**：i32.load、i32.store、i32.load8_s/u、i32.load16_s/u、i32.store8、i32.store16、memory.size、memory.grow
- **数值运算**：i32.const、i32.add/sub/mul/div_s/div_u、i32.rem_s/rem_u、i32.and/or/xor/shl/shr_s/shr_u
- **比较运算**：i32.eqz、eq、ne、lt_s/lt_u、gt_s/gt_u、le_s/le_u、ge_s/ge_u
- **宿主函数调用**：print、println、putchar、getchar、meminfo、time、alloc、free

### WASM 系统调用桥接

| 宿主函数 ID | 名称 | 参数 | 说明 |
|-------------|------|------|------|
| 0 | print | ptr, len | 打印字符串 |
| 1 | println | ptr, len | 打印字符串+换行 |
| 2 | putchar | char | 输出单字符 |
| 3 | getchar | 无 | 阻塞读取字符 |
| 4 | meminfo | 无 | 内存信息 |
| 5 | time | 无 | 系统时间 |
| 6 | alloc | size | 分配内存 |
...
内核内置多个 WASM 应用，通过 Shell 命令运行：

| 命令 | 说明 | 期望输出 |
|------|------|----------|
| `wasmtest2` | 常量测试 | `Result: 42` |
| `wasmtest3` | loop 倒计时 | `Result: 0` |
| `wasmtest4` | 系统调用测试 | `Result: 0` (打印 'A') |
| `wasmtest5` | br 控制流测试 | `br_table result = 20` |
| `wasmtest6` | 字节内存操作 | `store8/load8 result = 42` |
| `wasmapp fibonacci` | Fibonacci 数列 | `fib(10) = 55` |
| `wasmapp factorial` | 阶乘计算 | `5! = 120` |
| `wasmapp multiply` | 乘法运算 | `7 * 8 = 56` |
| `wasmapp countdown` | 倒计时循环 | `Countdown result = 0` |
